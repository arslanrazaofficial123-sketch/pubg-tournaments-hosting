...
export async function sendOrderConfirmationEmail(to: string | null | undefined, order: OrderEmailData): Promise<boolean> {
  ...
  try {
    ...
    await transporter.sendMail({
      ...
      to: recipient,
      subject: `Order Confirmation - ${order.packageLabel} (${order.ucAmount} UC)`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
        ...
        <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #1f2937; max-width: 600px; margin: 0 auto; padding: 20px;">
          ...
          <div style="background: #f9fafb; padding: 30px; border-radius: 0 0 12px 12px; border: 1px solid #e5e7eb; border-top: none;">
            ...
            <p style="color: #9ca3af; font-size: 12px; text-align: center;">
              Epix Esports • Sar Suba Shah Tehsil Kallar Syedan, Rawalpindi, Pakistan
            </p>
          </div>
        </body>
        </html>
...
export async function sendAdminOrderNotificationEmail(order: OrderEmailData): Promise<boolean> {
  try {
    ...
    await transporter.sendMail({
      from: `"Epix Esports" <${env.smtpUser}>`,
      to: adminEmail,
      subject: `🔔 New Shop Order - ${order.packageLabel} (${order.ucAmount} UC)`,
      html: `
        <!DOCTYPE html>
        <html>
        <head>
